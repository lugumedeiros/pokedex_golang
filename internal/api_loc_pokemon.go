...
func getCatchChance(trainer_xp float64, pokemon_xp float64) float64 {
	r := (trainer_xp / pokemon_xp) - 0.7
	if r < 0.0 {
		return 0.0
	}
	chance := math.Pow(r / 0.7, 0.38)
	return chance
}

func GetPokemon(name string) (success bool, err bool) {
	...
	defer res.Body.Close()
	decoder := json.NewDecoder(res.Body)
	err_dec := decoder.Decode(&pokemon)
	if err_dec != nil {
		return false, true
	}
